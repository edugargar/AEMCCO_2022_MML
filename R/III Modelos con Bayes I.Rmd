...
```{r}
????(bay_mod_1_prior_post, bay_mod_1_prior2_post)
```
...
Ahora queremos realizar los BF para los parámetros del modelo solo con información a priori y a posteriori con respecto a un valor nulo = 0 y el BF del modelo a posterori con respecto a que toma una dirección positiva.
```{r}
bayesfactor_parameters(???)
bayesfactor_parameters(?????)
bayesfactor_parameters(????, direction = ??)
```

Por último, ¿podríamos utilizar esta estragia para comparar modelos?¿cómo se interpretaría?
```{r}
??????(bay_mod_1_prior_post, bay_mod_1_prior2_post)
```
