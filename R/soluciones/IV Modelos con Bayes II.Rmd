...
bay_mod_1_null<- brm(calificacion ~ 1 + (1|club_baile), 
                        data = data, 
                        warmup = 1000,
                        iter = 2000, 
...
```{r}
model_parameters(bay_mod_1_null)
```

```{r}
pp_check(bay_mod_1_null)
```
...
```{r}
bay_mod_1_null_post <- update(bay_mod_1_null,
                                sample_prior = "no",
                                save_pars = save_pars(all = TRUE), 
                                seed = 6)
```
...
Un aspecto clave es conocer el valor del ICC. Podemos utilizar la función `icc` o la función `variance-decomposition`. Personalmente, me parece que esta segunda es mucho más clara para ver los efectos asociados a cada nivel.

```{r}
variance_decomposition(bay_mod_1_null_post)
```
...
hypothesis(bay_mod_1_null_post, hyp, class = NULL)
```
...
```{r}
compare_performance(bay_mod_1_prior2_post, 
                    bay_mod_1_null_post)
```
...
prior_user_inf <- c(set_prior("normal(5,2)", class = "Intercept"),
  ...
  bay_mod_2_ri<- brm(calificacion ~ entrenamiento_within + (1|club_baile), 
                        data = data, 
                        warmup = 1000,
                        iter = 2000, 
                        prior = prior_user_inf,
...
describe_posterior(bay_mod_2_ri,
  centrality = "mean",
  ci_method = "HDI",
  diagnostic = "all")
```

Calculamos la distribución a posteriori con nuestros datos y la función `update`

```{r}
bay_mod_2_ri_post <- update(bay_mod_2_ri,
                                sample_prior = "no",
                                save_pars = save_pars(all = TRUE), 
                                seed = 6)
...
```{r}
bay_mod_2_def<- brm(calificacion ~ entrenamiento_within + (1|club_baile), 
                        data = data, 
                        warmup = 1000,
                        iter = 2000, 
                        save_pars = save_pars(all = TRUE),
