...
```{r}
newdata_mod4 <- expand_grid(???? = countries$country,
                            year_within =????) %>% 
  left_join(countries, by = "country")

pred_mod4 <- mod_4_rs %>%
  ???(newdata = newdata_mod4) %>% 
...
```{r}
mod_6_rs_nested <- lmer(lifeExp ~ year_within + (1|year) + (????), 
                      data = data)
...
¿Y si quisiéramos obtener los efectos aleatorios obtenidos?
...
```{r}
mod_7_ri_context <- lmer(lifeExp ~ ?????+ (1 | country), 
                      data = data)
